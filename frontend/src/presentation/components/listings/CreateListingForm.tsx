    ...
    <Dialog open={open} onClose={onClose} className="relative z-50">
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <DialogPanel className="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl">
          <DialogTitle className="text-lg font-semibold text-gray-900">Create Listing</DialogTitle>
          <form onSubmit={handleSubmit} className="mt-4 space-y-3">
            <div>
              ...
              <input required value={domainName} onChange={(e) => setDomainName(e.target.value)}
                className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm" placeholder="example.com" />
            </div>
            <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="flex items-center gap-2">
                ...
                <label className="text-sm text-gray-700">Auto-Extend</label>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">Window (s)</label>
                <input type="number" value={autoExtWindowSec} onChange={(e) => setAutoExtWindowSec(Number(e.target.value))}
                  className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">Extension (s)</label>
