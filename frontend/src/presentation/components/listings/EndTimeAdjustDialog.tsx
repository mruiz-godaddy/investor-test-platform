    ...
    <Dialog open={open} onClose={onClose} className="relative z-50">
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <DialogPanel className="w-full max-w-sm rounded-xl bg-white p-6 shadow-xl">
          <DialogTitle className="text-lg font-semibold text-gray-900">Adjust End Time</DialogTitle>
          <p className="mt-1 text-sm text-gray-500">{listing.domainName}</p>
          <div className="mt-4 flex gap-2">
            <button onClick={() => setMode('relative')}
              className={`rounded-md px-3 py-1 text-sm ${mode === 'relative' ? 'bg-indigo-600 text-white' : 'bg-gray-100'}`}>
              + Seconds
            </button>
            <button onClick={() => setMode('absolute')}
              className={`rounded-md px-3 py-1 text-sm ${mode === 'absolute' ? 'bg-indigo-600 text-white' : 'bg-gray-100'}`}>
              Absolute
            </button>
          </div>
          <div className="mt-4">
            {mode === 'relative' ? (
              <div>
                <label className="block text-sm font-medium text-gray-700">Add Seconds</label>
