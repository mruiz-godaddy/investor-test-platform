...
function BidTable({ title, bids, colorClass }: { title: string; bids: ShopperBid[]; colorClass: string }) {
  if (bids.length === 0) {
    return (
      <div className="mt-4">
        <h3 className="text-sm font-semibold text-gray-900">{title} <span className={`ml-1 inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${colorClass}`}>0</span></h3>
        <p className="mt-2 text-sm text-gray-500">No bids.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mt-4">
      <h3 className="text-sm font-semibold text-gray-900">{title} <span className={`ml-1 inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${colorClass}`}>{bids.length}</span></h3>
      <div className="mt-2 overflow-x-auto rounded-lg border border-gray-200">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="divide-y divide-gray-200 bg-white">
            {bids.map((bid) => (
              <tr key={bid.bidId}>
                <td className="px-3 py-2 text-xs text-gray-900 font-medium">{bid.domainName || '-'}</td>
...
export default function ShopperDetailPage() {
  ...
  return (
    <div>
      ...
      <h2 className="mt-4 text-xl font-bold text-gray-900">{shopper.displayName || shopper.shopperId}</h2>

      <div className="mt-4 grid grid-cols-2 gap-6">
        <div className="rounded-lg border border-gray-200 bg-white p-4">
          <h3 className="text-sm font-semibold text-gray-900">Shopper Info</h3>
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-4">
          <h3 className="text-sm font-semibold text-gray-900">Bid Summary</h3>
          <dl className="mt-3 space-y-2 text-sm">
            {[
      ...
      <BidTable title="Active Bids" bids={activeBids} colorClass="bg-blue-100 text-blue-800" />
      <BidTable title="Won Bids" bids={wonBids} colorClass="bg-green-100 text-green-800" />
      <BidTable title="Lost Bids" bids={lostBids} colorClass="bg-red-100 text-red-800" />
    </div>
  );
}
